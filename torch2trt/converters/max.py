...
def test_max_reduce_dim1_val():
    return TestInterface(lambda x: torch.max(x, 1)[0])

@add_module_test(torch.float32, torch.device('cuda'), [(1, 3)])
@add_module_test(torch.float32, torch.device('cuda'), [(1, 3, 3)])
def test_max_reduce_dim1_idx():
    return TestInterface(lambda x: torch.max(x, 1)[1])

@add_module_test(torch.float32, torch.device('cuda'), [(1, 3, 3)])
def test_max_reduce_dim2_val():
    return TestInterface(lambda x: torch.max(x, 2)[0])

@add_module_test(torch.float32, torch.device('cuda'), [(1, 3, 3)])
def test_max_reduce_dim2_idx():
    return TestInterface(lambda x: torch.max(x, 2)[1])

@add_module_test(torch.float32, torch.device('cuda'), [(1, 3)])
@add_module_test(torch.float32, torch.device('cuda'), [(1, 3, 3)])
def test_max_reduce_dim1_keepdim_val():
    return TestInterface(lambda x: torch.max(x, 1, keepdim=True)[0])    

@add_module_test(torch.float32, torch.device('cuda'), [(1, 3)])
@add_module_test(torch.float32, torch.device('cuda'), [(1, 3, 3)])
def test_max_reduce_dim1_keepdim_idx():
    return TestInterface(lambda x: torch.max(x, 1, keepdim=True)[1])  
